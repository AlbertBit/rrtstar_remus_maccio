...
% Path 
% 4-> 21,22,23 check
path_id= fopen(strcat('./../Datalog/Planner/Time/Obstacle15/0optTraj.txt')); 
...
NumObtacleRegion=max(size(obstacles(:,1)))-1;

for j=1:NumObtacleRegion 
obsCenter(j,:)= obstacles(j+1,1:3);
obsSize(j,:)=   obstacles(j+1,4:6)./2.0;
