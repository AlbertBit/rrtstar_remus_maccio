...
int main () {
    ...
    obstacle_file.open ((DataLogPath2+"/obstacle_timings10.txt").c_str(),ios::out);
    obstacle_file<<"object_size"<<'\t'<<"collision_rate"<<'\t'<<"elapsed_time"<<'\t'<<"path_size"<<endl;
    obstacle_file.close();



    for( int iter = 0; iter < N_OBSTACLE; iter++) {
        ...
        for( int sim = 0; sim < N_SIMULATIONS; sim++) {
            ...
            double rootCartSize[3] = {LENGTH, WIDTH, HEIGHT};
        ...
        ofstream obstacle_file;
        obstacle_file.open ((DataLogPath2+"/obstacle_timings10.txt").c_str(),ios::app);
        obstacle_file<<iter+1<<'\t'<<collision_rate/N_SIMULATIONS<<'\t'<<elapsed_time/N_SIMULATIONS<<endl;;
        obstacle_file.close();
    }
